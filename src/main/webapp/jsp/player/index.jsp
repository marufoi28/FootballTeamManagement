	...
	<%@ include file="../layout/header.jsp" %>
		<div class="dashboard-layout">
			<%@ include file="menu.jsp" %>
				<main class="main">
					<div class="search-and-file-area">
								...
								<table class="search-table">
									...
									<tbody>
										<tr>
											...
											<td><input type="search" class="search-name-input" id="searchName" name="searchName"
													value="${searchName}" placeholder="選手名を入力してください" /></td>
										</tr>
										<tr>
											<td><label>ポジション</label></td>
											<td>
												<c:forEach var="position" items="${positions}" varStatus="status">
													<input type="checkbox" class="search-position-input" name="searchPositionIdList"
														value="${position.getPositionId() }" <c:if
														test="${searchPositionIdList != null && searchPositionIdList.contains(position.getPositionId())}">checked
													</c:if>>
										...
										<tr>
											...
											<td>
												<label>
													<input type="radio" class="search-license-input" name="searchHasLicense" value="" <c:if
														test="${ searchHasLicense == null }">checked</c:if>>
													指定なし
												</label>
												<label>
													<input type="radio" class="search-license-input" name="searchHasLicense" value="1" <c:if
														test="${ searchHasLicense != null && searchHasLicense }">checked</c:if>>
													あり
												</label>
												<label>
													<input type="radio" class="search-license-input" name="searchHasLicense" value="0" <c:if
														test="${ searchHasLicense != null && !searchHasLicense }">checked</c:if>>
													なし
												</label>
											</td>
										...
										<tr>
											...
											<td>
												<label>
													<input type="radio" class="search-student-input" name="searchIsStudent" value="" <c:if
														test="${searchIsStudent == null }">checked</c:if>>
													指定なし
												</label>
												<label>
													<input type="radio" class="search-student-input" name="searchIsStudent" value="1" <c:if
														test="${searchIsStudent != null && searchIsStudent }">checked</c:if>>
													学生
												</label>
												<label>
													<input type="radio" class="search-student-input" name="searchIsStudent" value="0" <c:if
														test="${searchIsStudent != null && !searchIsStudent }">checked</c:if>>
													非学生
												</label>
											</td>
						...
						<div class="file-management-area noactive" id="fileManagementArea">
							<p class="menu-title">ファイル操作</p>
						<div class="file-operation-message-area">
							<p id="message" class="${not empty successMessage ? 'success' : not empty errorMessage ? 'error' : ''}">
							    <c:choose>
							        <c:when test="${not empty successMessage}">
							            成功：<c:out value="${successMessage}" />
							        </c:when>
							        <c:when test="${not empty errorMessage}">
							            エラー：<c:out value="${errorMessage}" />
							        </c:when>
							    </c:choose>
							</p>
						</div>
							<table class="file-table">
								...
								<tbody>
									<tr>
										...
										<td></td>
										<td>
										    <button onclick="downloadCSV()"  class="btn btn-file-export">
									...
									<tr>
										<td>取り込み</td>
										<td>
										  <form action="ImportCSVServlet" method="post" enctype="multipart/form-data">
										    <input type="file" name="csvFile" id="csvFile" required />
										</td>
										<td>
										    <button type="submit" class="btn btn-file-export">
					...
					<c:set var="baseUrl"
						value="SearchPlayerServlet?searchName=${searchName}&searchHasLicense=${searchHasLicense}&searchPosition=${searchPosition}" />
